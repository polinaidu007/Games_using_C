...
int processInput(Board *board, char inputChar)
{
	...
	*/
	if ((inputChar == 'w' || inputChar == 'W') && goDown(board))
		return 1;
	else if ((inputChar == 's' || inputChar == 'S') && goUp(board))
		return 1;
	else if ((inputChar == 'a' || inputChar == 'A') && goRight(board))
		return 1;
	else if ((inputChar == 'd' || inputChar == 'D') && goLeft(board))
		return 1;
	return 0;
}
...
void playMove(Board *board, User *user, char choice){
	int cnt = processInput(board, choice);
	modifyMoveCountBasedOnProccessInput(user, cnt);
